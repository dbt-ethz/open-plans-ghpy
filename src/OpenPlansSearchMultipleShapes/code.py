...
class OpenPlansSearch:
    ...
    def __init__(self, search_shape):
...
class OpenPlansSearchMultipleShapes(component):

    def RunScript(self, searchShapes):
        ...
        for shape in searchShapes:
            if shape.IsValid and shape.IsClosed:

                op = OpenPlansSearch(shape)
                ...
                if op.openPlansItems:
                    projects.extend([OrderedDict((key, value) for key, value in item.__dict__.items() if key != 'height_mm') for item in op.openPlansItems])
